...
$sourceFolder = Join-Path $projectParent "wuma_ai_v1_3_2"
...
if (-not (Test-Path $sourceFolder)) {
    Write-Host "Cannot find wuma_ai_v1_3_2 beside this folder." -ForegroundColor Red
    exit 1
}
...
if (Test-Path $targetEnv) {
    ...
    Write-Host "Copied .env from v1.3.2." -ForegroundColor Green
} else {
    Write-Host "No .env found in v1.3.2. Create it from .env.example." -ForegroundColor Yellow
}
...
if (Test-Path $targetDatabase) {
    ...
    Write-Host "Copied the database from v1.3.2." -ForegroundColor Green
} else {
    Write-Host "No v1.3.2 database found. v1.3.4 will create a new one." -ForegroundColor Yellow
}
