...
$sourceFolder = Join-Path $projectParent "wuma_ai_v1_3_4"
...
if (-not (Test-Path $sourceFolder)) {
    Write-Host "Cannot find wuma_ai_v1_3_4 beside this folder." -ForegroundColor Red
    exit 1
}

if (Test-Path $targetEnv) {
    Write-Host "The v1.3.5 .env already exists. It was not overwritten." -ForegroundColor Yellow
} elseif (Test-Path $sourceEnv) {
    Copy-Item $sourceEnv $targetEnv
    Write-Host "Copied .env from v1.3.4." -ForegroundColor Green
} else {
    Write-Host "No .env found in v1.3.4. Create it from .env.example." -ForegroundColor Yellow
}

if (Test-Path $targetDatabase) {
    Write-Host "The v1.3.5 database already exists. It was not overwritten." -ForegroundColor Yellow
    ...
    Write-Host "Copied the database from v1.3.4." -ForegroundColor Green
} else {
    Write-Host "No v1.3.4 database found. v1.3.5 will create a new one." -ForegroundColor Yellow
}
